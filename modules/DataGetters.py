...
class get_footballPlayer_data_from_sofifa(Data_getter):
	...
	def getter_value(self):
		return self.main_table.find('div' , class_ = 'card-body stats').find_all('div' , class_ = 'column col-4 text-center')[2].find('span').text.strip()

	@property
	def getter_wage(self):
		 return self.main_table.find('div' , class_ = 'card-body stats').find_all('div' , class_ = 'column col-4 text-center')[3].find('span').text.strip()
