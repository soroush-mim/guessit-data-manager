...
def get_footballTeam_data_from_sofifa(attribute):
    ...
    def players(page, test=False):
        pattern = r'.{3,30}'
        values = [tag.find('a', {'href': re.compile('/player/[0-9]*')})['title'] for tag in
                  page.find_all('tr', {'class': 'starting'}) + page.find_all('tr', {'class': 'sub'}) + page.find_all(
                      'tr', {'class': 'res'})]
    ...
    def thirdkit(page, test=False):
        pattern = r'.{3,30}'
        value = [tag for tag in page.find_all('img', {'src': re.compile('.*?/kits/.*')}) if
                 tag.parent.text.find('Third') != -1][0]['src']
    ...
    def goalkeeperkit(page, test=False):
        pattern = r'.{3,30}'
        value = [tag for tag in page.find_all('img', {'src': re.compile('.*?/kits/.*')}) if
                 tag.parent.text.find('Goalkeeper') != -1][0]['src']
    ...
    def sofifaattackrate(page, test=False):
        pattern = r'.{3,30}'
        value = int([tag for tag in page.select('div.card-body.stats')[0].find('div').select('div') if
                     tag.text.find('Attack') != -1][0].find('span').text)
    ...
    def sofifamidfieldrate(page, test=False):
        pattern = r'.{3,30}'
        value = int([tag for tag in page.select('div.card-body.stats')[0].find('div').select('div') if
                     tag.text.find('Midfield') != -1][0].find('span').text)
    ...
    def sofifaoverallrate(page, test=False):
        pattern = r'.{3,30}'
        value = int([tag for tag in page.select('div.card-body.stats')[0].find('div').select('div') if
                     tag.text.find('Overall') != -1][0].find('span').text)
    ...
    def sofifadefencerate(page, test=False):
        pattern = r'.{3,30}'
        value = int([tag for tag in page.select('div.card-body.stats')[0].find('div').select('div') if
                     tag.text.find('Defence') != -1][0].find('span').text)

        return value if not test or re.search(pattern, value) else None

    def sofifadefencerate(page, test=False):
        pattern = r'.{3,30}'
        value = [tag for tag in page.select('div.card-body.stats')[0].find('div').select('div') if
                 tag.text.find('Defence') != -1][0].find('span').text
...
def get_actor_data_from_imdb(attribute):
    def name(page, test=False):
        return str(page.find('div', {'id': 'name-overview-widget'}).find('h1').find('span', {
            'class': 'itemprop'}).decode_contents(formatter="html")).strip()
    ...
    def birthdate(page, test=False):
        try:
            return re.search('(.*?)-([0-9]*)-([0-9]*)',
                             page.find('h4', text='Born:').parent.find('time')['datetime']).group(1) + '_' + re.search(
                '(.*?)-([0-9]*)-([0-9]*)', page.find('h4', text='Born:').parent.find('time')['datetime']).group(2) \
                   + '_' + re.search('(.*?)-([0-9]*)-([0-9]*)',
                                     page.find('h4', text='Born:').parent.find('time')['datetime']).group(3)
        except:
            return '##'
    ...
    def deathdate(page, test=False):
        try:
            return re.search('(.*?)-([0-9]*)-([0-9]*)',
                             page.find('h4', text='Died:').parent.find('time')['datetime']).group(1) + '_' + re.search(
                '(.*?)-([0-9]*)-([0-9]*)', page.find('h4', text='Died:').parent.find('time')['datetime']).group(2) \
                   + '_' + re.search('(.*?)-([0-9]*)-([0-9]*)',
                                     page.find('h4', text='Died:').parent.find('time')['datetime']).group(3)
        except:
            return '##'
    ...
    def movies(page, test=False):
        ...
        page = make_soup(
            'https://imdb.com' + page.find('a', {'href': re.compile('/name/.*?/?nmdp=1&ref_=nm_ql_4#filmography')})[
                'href'])
        return [tag.find('b').find('a').text for tag in
                page.find('div', {'id': 'filmography'}).find('div', {'class': 'filmo-category-section'}).find_all('div',
                                                                                                                  {
                                                                                                                      'class': 'filmo-row odd'})
                if not re.search('.*?\(.*?\)', tag.text)] \
               + [tag.find('b').find('a').text for tag in
                  page.find('div', {'id': 'filmography'}).find('div', {'class': 'filmo-category-section'}).find_all(
                      'div', {'class': 'filmo-row even'}) if not re.search('.*?\(.*?\)', tag.text)]
    ...
    def pictures(page, test=False):
        try:
            ...
            if page.find_all('a', {'href': re.compile('.*?/mediaindex.page.*')}):
                pages += list(set(['https://www.imdb.com' + tag['href'] for tag in
                                   page.find_all('a', {'href': re.compile('.*?/mediaindex.page.*')})]))
            pages = [make_soup(page) for page in pages[0:1]]
            pic_pages = list(itertools.chain.from_iterable([['https://www.imdb.com' + tag['href'] for tag in
                                                             page.find_all('a',
                                                                           {'href': re.compile('.*?/mediaviewer/.*')})]
                                                            for page in pages]))
    ...
    def height(page, test=False):
        try:
            return float(re.search('.*?"\((.*?)m\)',
                                   page.find('div', {'id': 'details-height'}).text.strip().replace('\xa0', '').replace(
                                       '\n', '')).group(1))
        except:
            return 0
    ...
    def spouse(page, test=False):
        ...
        if page.find('table', {'id': 'tableSpouses'}):
            return re.search('(.*?)\(.*\).*',
                             page.find('table', {'id': 'tableSpouses'}).text.strip().replace('\n', '').replace('  ',
                                                                                                               '')).group(
                1)
        else:
            return []
...
def get_director_data_from_imdb(attribute):
    ...
    def nick_name(page, test=False):
        try:
            page = make_soup('https://imdb.com' + page.find('a', text='Biography')['href'])
            return [tag.findNext().text.strip() for tag in page.find_all('td', {'class': 'label'}) if
                    tag.text.find('Nickname') != -1]
        except:
            return '##'
    ...
    def height(page, test=False):
        try:
            page = make_soup('https://imdb.com' + page.find('a', text='Biography')['href'])
            return float(re.search('.*\((.*?)m\)', [tag.findNext().text.strip().replace('\xa0', '') for tag in
                                                    page.find_all('td', {'class': 'label'}) if
                                                    tag.text.find('Height') != -1][0]).group(1))
        except:
            return 0
    ...
    def spouse(page, test=False):
        try:
            ...
            return [tag.find('td').text.strip() for tag in
                    spouse.find_all('tr', {'class': 'soda odd'}) + spouse.find_all('tr', {'class': 'soda even'})]
        except:
            return []
...
def get_movie_data_from_imdb(attribute):
    ...
    def name(page, test=False):
        ...
        page2 = make_soup(page.find('link', {'rel': 'canonical'})['href'] +
                          page.find('h2', text='Details').parent.find('a', {'href': re.compile('.*?releaseinfo.*')})[
                              'href'])
        try:
    ...
    def release_day(page, test=False):
        try:
            return int(
                page.select('div.title_wrapper')[0].find('a', {'href': re.compile('.*?releaseinfo.*')}).text.split(' ')[
                    0])
        except:
            return '##'
    ...
    def imdb_popularity(page, test=False):
        popularity_tag = [tag for tag in page.select('div.titleReviewBar')[0].select('div') if
                          tag.text.strip() == 'Popularity']
        if popularity_tag:
    ...
    def imdb_rank(page, test=False):
        if page.select('div.titleAwardsRanks'):
            return re.search('.*?#([0-9]*).*?', page.select('div#titleAwardsRanks')[0].find('a', {
                'href': re.compile('/chart/top.*')}).text).group(1)
        else:
            return 10 ** 10
    ...
    def imdb_user_reviews(page, test=False):
        try:
            return int(re.search(r'(.*?) .*?',
                                 page.find('div', {'class': 'titleReviewBarItem titleReviewbarItemBorder'}).find('a', {
                                     'href': re.compile('reviews.*?')}).text.strip().replace(',', '')).group(1))
        except:
            return 0
    ...
    def imdb_critic_reviews(page, test=False):
        try:
            return int(re.search(r'(.*?) .*?',
                                 page.find('div', {'class': 'titleReviewBarItem titleReviewbarItemBorder'}).find('a', {
                                     'href': re.compile('externalreviews.*?')}).decode_contents(
                                     formatter="html").strip().replace(',', '')).group(1))
        except:
            return 0
    ...
    def summaries(page, test=False):
        ...
        return [tag.find('p').text.strip() for tag in
                page.find('ul', {'id': 'plot-summaries-content'}).find_all('li', {'class': 'ipl-zebra-list__item'})]

    def summary(page, test=False):
        page = make_soup('https://www.imdb.com' + page.find('a', text='Plot Summary')['href'])
        return [tag.find('p').text.strip() for tag in
                page.find('ul', {'id': 'plot-summaries-content'}).find_all('li', {'class': 'ipl-zebra-list__item'})][0]

    def directors(page, test=False):
        try:
            page = make_soup('https://www.imdb.com' + page.find('a', text='Full Cast and Crew')['href'])
            return list(set([tag.find('td').text.strip() for tag in
                             [tag for tag in page.select('h4') if tag.text.strip() == 'Directed by'][0].findNext(
                                 'table').find_all('tr') if tag.find('td').text.strip() != '']))
        except:
            return []
    ...
    def writers(page, test=False):
        ...
        try:
            page = make_soup('https://www.imdb.com' + page.find('a', text='Full Cast and Crew')['href'])
            return list(set([tag.find('td').text.strip() for tag in
                             [tag for tag in page.select('h4') if tag.text.strip() == 'Writing Credits'][0].findNext(
                                 'table').find_all('tr') if tag.find('td').text.strip() != '']))
        except:
            return []
    ...
    def stars(page, test=False):
        return [tag.text.strip() for tag in
                page.find('h4', text='Stars:').parent.find_all('a', {'href': re.compile('.*/name/.*')})]

    def cast(page, test=False):
        page = make_soup('https://www.imdb.com' + page.find('a', text='Full Cast and Crew')['href'])
        return [tag.text.strip().replace('\n', '').replace('  ', '') for tag in
                page.find('table', {'class': 'cast_list'}).find_all('a') if tag.text != '']

    def characters(page, test=False):
        page = make_soup('https://www.imdb.com' + page.find('a', text='Full Cast and Crew')['href'])
        return [tag.text.strip().replace('\n', '').replace('  ', '') for tag in
                page.select('table.cast_list')[0].select('td.character')]
    ...
    def videos(page, test=False):
        try:
            ...
            if page.find_all('a', {'href': re.compile('.*?/videogallery.page.*')}):
                pages += list(set(['https://www.imdb.com' + tag['href'] for tag in
                                   page.find_all('a', {'href': re.compile('.*?/videogallery.page.*')})]))
            pages = [make_soup(page) for page in pages[0:1]]
            video_pages = list(itertools.chain.from_iterable([['https://www.imdb.com' + tag['href'] for tag in
                                                               page.find_all('a', {
                                                                   'href': re.compile('.*?/videoplayer/.*')})] for page
                                                              in pages]))
    ...
    def quotes(page, test=False):
        try:
            ...
            return [tag.find('div').text.strip().replace('\n', '') for tag in
                    page.find_all('div', {'class': 'quote soda sodavote odd'}) + page.find_all('div', {
                        'class': 'quote soda sodavote even'})]
        except:
            return []
    ...
    def filming_locations(page, test=False):
        try:
            ...
            page = make_soup(x)
            return [tag.find('a').text.strip() for tag in
                    page.find('section', {'id': 'filming_locations'}).find_all('div', {
                        'class': 'soda sodavote odd'}) + page.find_all('div', {'class': 'soda sodavote even'})]
        except:
            return []
    ...
    def tag_lines(page, test=False):
        try:
            ...
            page = make_soup(x)
            return [tag.text.strip() for tag in
                    page.find('div', {'id': 'taglines_content'}).find_all('div', {'class': 'soda odd'}) + page.find_all(
                        'div', {'class': 'soda even'})]
        except:
            return []
    ...
    def keywords(page, test=False):
        try:
            ...
            return list(itertools.chain.from_iterable(
                [[tag.text.strip() for tag in tag.find_all('div', {'class': 'sodatext'})] for tag in
                 page.find('div', {'id': 'keywords_content'}).find_all('tr', {'class': 'odd'}) + page.find_all('tr', {
                     'class': 'even'})]))
        except:
            return []
    ...
    def trivia(page, test=False):
        try:
            page = make_soup('https://imdb.com' + page.find('a', text='Trivia')['href'])
            return [tag.find('div').text.strip() for tag in
                    page.find_all('div', {'class': 'soda odd sodavote'}) + page.find_all('div', {
                        'class': 'soda even sodavote'})]
        except:
            return []
...
def get_book_data_from_goodreads(attribute):
    def book_list(page, test=False):
        page = make_soup(page)
        return list(set([re.search(r'.*?/book/show/(.*)', tag['href']).group(1) for tag in
                         page.find_all('a', {'href': re.compile('.*?/book/show/(.*)')})]))
    ...
    def related(page, test=False):
        return [tag.find('img')['alt'] for tag in page.find_all('a', {'href': re.compile('/list/show/.*')}) if
                tag.find('img')]
...
def get_author_data_from_goodreads(attribute):
    def author_list(page, test=False):
        page = make_soup(page)
        return list(set([re.search(r'.*?/author/show/(.*)', tag['href']).group(1) for tag in
                         page.find_all('a', {'href': re.compile('.*?/author/show/(.*)')})]))
    ...
    def goodreadsVotes(page, test=False):
        return int(re.search('.*?([0-9]*).*', page.find('a', {
            'href': re.compile('/review/list/.*sort.rating.view.reviews')}).text.strip()).group(1))

    def goodreadsReviews(page, test=False):
        return int(re.search('.*?([0-9]*).*', page.find('a', {
            'href': re.compile('/review/list/.*sort.review.view.reviews')}).text.strip()).group(1))

    def books(page, test=False):
        page = make_soup('https://www.goodreads.com' +
                         [tag['href'] for tag in page.find_all('a', {'class': 'actionLink'}) if
                          tag.text.find('More books by ') != -1][0])
        return [tag.text.strip() for tag in
                page.find('table', {'class': 'tableList'}).find_all('a', {'class': 'bookTitle'})]
    ...
    def goodreadsFolowers(page, test=False):
        return int(re.search('.*?\(([0-9,]*)\).*?',
                             page.find('a', {'href': re.compile('/author_followings.id..*.method.get')}).text).group(
            1).replace(',', ''))

    def relatedAuthors(page, test=False):
        page = make_soup('https://goodreads.com' + page.find('a', {'href': re.compile('/author/similar/.*')})['href'])
        return [tag.find('a', {'href': re.compile('/author/show/.*')}).text.strip() for tag in
                page.select('ul.list')[1].select('div.readable.description')]
    ...
    def favoriteBooks(page, test=False):
        page = make_soup('https://www.goodreads.com' +
                         [tag['href'] for tag in page.find_all('a', {'class': 'actionLink right'}) if
                          tag.text.find('More of ') != -1][0])
        return [tag.find('td', {'class': 'field title'}).find('a').text.strip() for tag in
                page.find_all('tr', {'class': 'bookalike review'})]
...
def get_country_data_from_cia(attribute, page):
    ...
    def flag(page, test=False):
        return 'https://www.cia.gov/library/publications/the-world-factbook/' + \
               page.find('img', {'src': re.compile('.*?/graphics/flags/large/.*?.gif')})['src'][3:]

    def pictures(page, test=False):
        return ['https://www.cia.gov/library/publications/the-world-factbook/' + tag['href'][3:] for tag in
                page.find_all('a', {'href': re.compile('.*?/photo_gallery/[a-z]{2}/images/large/.*?\.jpg')})]

    def location_picture(page, test=False):
        return 'https://www.cia.gov/library/publications/the-world-factbook/' + \
               page.find('img', {'src': re.compile('.*?/graphics/locator/.*?/.*?_large_locator.gif')})['src'][3:]

    def map(page, test=False):
        return 'https://www.cia.gov/library/publications/the-world-factbook/' + \
               page.find('img', {'src': re.compile('.*?/graphics/maps/[a-z]{2}-map.gif')})['src'][3:]

    def location_description(page, test=False):
        return [tag.parent.find_next_sibling("div").text.strip() for tag in page.find_all('a') if
                tag.text.strip() == 'Location:'][0]

    def area(page, test=False):
        return [re.search('([0-9]*) .*',
                          tag.find_next_sibling('div').find('span', {'class': 'category_data'}).text.strip().replace(
                              ',', '')).group(1) for tag in page.find_all('div') if tag.text.strip() == 'Area:'][0]

    def population(page, test=False):
        return \
        [re.search('([0-9]*) .*', tag.parent.find_next_sibling("div").text.strip().replace(',', '')).group(1) for tag in
         page.find_all('a') if tag.text.strip() == 'Population:'][0]

    def borderCountreis(page, test=False):
        return [tag.findNext().text for tag in page.find_all('span', {'class': 'category'}) if
                tag.text.find('border countries') != -1]
    ...
    def lowestPoint(page, test=False):
        return [tag.findNext().text.replace('lowest point:', '') for tag in page.find_all('span', {'class': 'category'})
                if tag.text.find('elevation extremes: ') != -1]

    def highestPoint(page, test=False):
        return [tag.findNext().findNext().text.replace('highest point: ', '') for tag in
                page.find_all('span', {'class': 'category'}) if tag.text.find('elevation extremes: ') != -1]
    ...
    def populationGrowthRate(page, test=False):
        return re.search('(.*?)%.*',
                         page.find('a', text='Population growth rate:').findNext().findNext().findNext().text).group(1)
...
def get_people_data_from_biography(attribute):
    def people_list(page, test=False):
        page = make_soup(page)
        return list(set([re.search(r'.*/people/(.*)', tag['href']).group(1) for tag in
                         page.find_all('a', {'href': re.compile('.*/people/.*')})]))
...
def get_anime_data_from_myanimelist(attribute, page):
    def anime_list(page, test=False):
        page = make_soup(page)
        return list(set([re.search(r'.*/anime/([0-9]*?/[^/]*)', tag['href']).group(1) for tag in
                         page.find_all('a', {'href': re.compile('.*/anime/([0-9]*?/[^/]*)')})]))

    def name(page, test=False):
        return page.find('span', {'itemprop': 'name'}).text.strip()

    def nameJapanese(page, test=False):
        return [re.search('<span.*</span>(.*)', str(tag.parent)).group(1) for tag in page.find_all('span') if
                tag.text.strip() == 'Japanese:'][0].strip()

    def nameEnglish(page, test=False):
        return [re.search('<span.*</span>(.*)', str(tag.parent)).group(1) for tag in page.find_all('span') if
                tag.text.strip() == 'English:'][0].strip()
...
def get_word_data_from_merriam(attribute, page):
    if attribute == 'word_list':
        page = make_soup(page)
        return list(set([re.search(r'/dictionary/(.*)', tag['href']).group(1) for tag in
                         page.find_all('a', {'href': re.compile(r'/dictionary/(.*)')})]))
    if attribute == 'word':
    ...
    if attribute == 'examples':
        return [tag.text for tag in page.find('div', {'class': 'card-primary-content def-text'}).find_all('p', {
            'class': 'definition-inner-item'})]
...
def get_volleyballTeam_data_from_volleyballWorld(attribute, page):
    if attribute == 'volleyballTeam_list':
        page = make_soup(page)
        return list(set([re.search(r'/en/men/teams/(.*)', tag['href']).group(1) for tag in
                         page.find_all('a', {'href': re.compile(r'/en/men/teams/(.*)')})]))
    if attribute == 'coach':
        return [tag.text for tag in page.find('div', {'class': 'details'}).find_all('h4')]
    if attribute == 'teamManager':
        return [tag.parent.find('strong').text for tag in
                page.find('ul', {'class': 'line-list'}).find_all('span', {'class': 'role'}) if
                tag.text.find('Team Manager') != -1]
    if attribute == 'assistantCoach':
        return [tag.parent.find('strong').text for tag in
                page.find('ul', {'class': 'line-list'}).find_all('span', {'class': 'role'}) if
                tag.text.find('Assistant coach') != -1]

    if attribute == 'secondAssistantCoach':
        return [tag.parent.find('strong').text for tag in
                page.find('ul', {'class': 'line-list'}).find_all('span', {'class': 'role'}) if
                tag.text.find('Second Assistant Coach') != -1]

    if attribute == 'Doctor':
        return [tag.parent.find('strong').text for tag in
                page.find('ul', {'class': 'line-list'}).find_all('span', {'class': 'role'}) if
                tag.text.find('Doctor') != -1]

    if attribute == 'players':
        page = make_soup(
            'http://www.volleyball.world/' + page.find('a', {'href': re.compile('/en/men/teams/.*?/players')})['href'])
        return [tag.text.strip() for tag in page.find_all('a', {'href': re.compile('/en/men/teams/.*?/players.*')})][
               5::3]
    if attribute == 'mostAge':
        x = 'http://www.volleyball.world' + page.find('a', {'href': re.compile('en/men/teams/.*/facts_and_figures')})[
            'href']
        page = make_soup(x)
...
def get_celebrity_data_from_theFamousPeople(attribute, page):
    if attribute == 'celebrity_list':
        page = make_soup(page)
        return list(set([re.search(r'/profiles/(.*).php', tag['href']).group(1) for tag in
                         page.find_all('a', {'href': re.compile(r'/profiles/(.*).php')})]))
    if attribute == 'name':
        return page.find('h1').text.replace(' Biography', '')
    if attribute == 'birthDay':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('p', {'class': 'quickfactsdata'}) if
         tag.text.find('Birthday') != -1][0].text.replace('Birthday: ', '')

    if attribute == 'nationality':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('p', {'class': 'quickfactsdata'}) if
         tag.text.find('Nationality') != -1][0].find('a').text

    if attribute == 'sunSing':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('Sun Sing') != -1][0].parent.text.replace('Sun Sign: ', '')
    if attribute == 'deathdate':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('p', {'class': 'quickfactsdata'}) if
         tag.text.find('Died on') != -1][0].text.replace('Died on: ', '')
    if attribute == 'bornIn':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('Born') != -1][0].parent.text.replace('Born in: ', '')
    if attribute == 'deathYear':
        return int(
            [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('p', {'class': 'quickfactsdata'})
             if tag.text.find('Died on') != -1][0].text.replace('Died on: ', '')[-4:])
    if attribute == 'birthyear':
        return int(
            [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('p', {'class': 'quickfactsdata'})
             if tag.text.find('Birthday') != -1][0].text.replace('Birthday: ', '')[-4:])
    if attribute == 'famousAs':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('Famous as') != -1][0].parent.text.replace('Famous as: ', '')
    if attribute == 'spouse':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('Spouse/Ex') != -1][0].parent.text.replace('Spouse/Ex-: ', '')
    if attribute == 'father':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('father') != -1][0].parent.text.replace('father: ', '')
    if attribute == 'mother':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('mother') != -1][0].parent.text.replace('mother: ', '')
    if attribute == 'siblings':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('siblings') != -1][0].parent.text.replace('siblings: ', '')
    if attribute == 'childrens':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('children') != -1][0].parent.text.replace('children: ', '')
    if attribute == 'religion':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('religion') != -1][0].parent.text.replace('religion: ', '')
    if attribute == 'deathAge':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('Died At Age') != -1][0].parent.text.replace('Died At Age: ', '')
    if attribute == 'age':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('p', {'class': 'quickfactsdata'}) if
         tag.text.find('Age') != -1][0].find('a').text[:2]
    if attribute == 'deathPlace':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('place of death') != -1][0].parent.text.replace('place of death: ', '')
    if attribute == 'personality':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('p', {'class': 'quickfactsdata'}) if
         tag.text.find('Personality') != -1][0].text.replace('Personality: ', '')
    if attribute == 'deathCause':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('p', {'class': 'quickfactsdata'}) if
         tag.text.find('Cause of Death') != -1][0].find('a', {'class': ''}).text
    if attribute == 'netWorth':
        return \
        [tag for tag in page.find('div', {'class': 'fps-desc fpf-block'}).find_all('span', {'class': 'quickfactstitle'})
         if tag.text.find('Net worth') != -1][0].parent.text.replace('Net worth: ', '')
    if attribute == 'photos':
        return [tag['src'] for tag in page.find('div', {'class': 'carousel-inner '}).find_all('img', {
            'class': 'carousel-desktop img-responsive hide_on_mobile'})]
...
def get_musicArtist_data_from_discogs(attribute, page):
    if attribute == 'musicArtist_list':
        page = make_soup(page)
        return list(set([re.search(r'/artist/(.*)', tag['href']).group(1) for tag in
                         page.find_all('a', {'href': re.compile(r'/artist/(.*)')})]))
    if attribute == 'name':
    ...
    if attribute == 'aliases':
        return [tag for tag in page.find('div', {'class': 'profile'}).find_all('div', {'class': 'head'}) if
                tag.text.find('Aliases') != -1][0].findNext().text.strip().replace(' \n                ', '')
    if attribute == 'inGroups':
        return [tag.findNext().text.strip().replace(' \n               ', '') for tag in
                page.find('div', {'class': 'profile'}).find_all('div', {'class': 'head'}) if
                tag.text.find('In Groups') != -1][0]
    if attribute == 'variations':
        return page.find('div', {'id': 'anvs'}).text.strip()
    if attribute == 'members':
        return [tag.findNext().text.strip().replace(' \n                   ', '') for tag in
                page.find('div', {'class': 'profile'}).find_all('div', {'class': 'head'}) if
                tag.text.find('Members') != -1][0]
    if attribute == 'photos':
...
def collect_footballTeam_id_from_sofifa(pages, data_count=700, timeout=10 ** 3, checked_id=[], checked_pages=[]):
    return collect_data_id_from_resource(pages, 'https://sofifa.com', '/team/([^/]*).*?', data_count=data_count,
                                         timeout=timeout, checked_id=checked_id, checked_pages=checked_pages,
                                         recursive=False)


def collect_footballPlayer_id_from_sofifa(pages, data_count=15000, timeout=10 ** 3, checked_id=[], checked_pages=[]):
    return collect_data_id_from_resource(pages, 'https://sofifa.com', '/player/([^/]*).*?', data_count=data_count,
                                         timeout=timeout, checked_id=checked_id, checked_pages=checked_pages,
                                         recursive=False)


def collect_author_id_from_goodreads(pages, data_count=10, timeout=10 ** 3, checked_id=[], checked_pages=[]):
    return collect_data_id_from_resource(pages, 'https://www.goodreads.com', '/author/show/([^/]*)$',
                                         data_count=data_count, timeout=timeout, checked_id=checked_id,
                                         checked_pages=checked_pages)


def collect_book_id_from_goodreads(pages, data_count=10, timeout=10 ** 3, checked_id=[], checked_pages=[]):
    return collect_data_id_from_resource(pages, 'https://www.goodreads.com', '/book/show/([0-9]*?-[^/]*)$',
                                         data_count=data_count, timeout=timeout, checked_id=checked_id,
                                         checked_pages=checked_pages)


def collect_country_id_from_cia(pages, data_count=10, timeout=10 ** 3, checked_id=[], checked_pages=[]):
    return collect_data_id_from_resource(pages, 'https://www.cia.gov/library/publications/the-world-factbook/geos',
                                         '/(.*?).html', data_count=data_count, timeout=timeout, checked_id=checked_id,
                                         checked_pages=checked_pages)


def collect_people_id_from_biography(pages, data_count=10, timeout=10 ** 3, checked_id=[], checked_pages=[]):
    return collect_data_id_from_resource(pages, 'https://biography.com', '/people/([^/]*).*?', data_count=data_count,
                                         timeout=timeout, checked_id=checked_id, checked_pages=checked_pages)


def collect_anime_id_from_myanimelist(pages, data_count=10, timeout=10 ** 3, checked_id=[], checked_pages=[]):
    return collect_data_id_from_resource(pages, 'https://myanimelist.net', '/anime/([^/]*).*?', data_count=data_count,
                                         timeout=timeout, checked_id=checked_id, checked_pages=checked_pages)


def collect_movie_id_from_imdb(pages, data_count=10, timeout=10 ** 3, checked_id=[], checked_pages=[]):
    for page in pages:
        checked_id += [re.search('/title/(tt[0-9]*).*?', tag['href']).group(1) for tag in
                       make_soup(page).find_all('a', {'href': re.compile('/title/.*?')})]
...
def collect_actor_id_from_imdb(pages, data_count=10, timeout=10 ** 3, checked_id=[], checked_pages=[]):
    for page in pages:
        checked_id += [re.search('/name/(nm[0-9]*)/.*?', tag['href']).group(1) for tag in
                       make_soup(page).find_all('a', {'href': re.compile('/name/.*?st.*?')})]
...
def collect_director_id_from_imdb(pages, data_count=10, timeout=10 ** 3, checked_id=[], checked_pages=[]):
    for page in pages:
        checked_id += [re.search('/name/(nm[0-9]*)/.*?', tag['href']).group(1) for tag in
                       make_soup(page).find_all('a', {'href': re.compile('/name/.*?dr.*?')})]
