	...
	with mp.Pool() as pool:
		...
		for resource in get_resources(db_name) if resources is None else resources:
			...
			while len(db) < max_db_all and count_find_all < max_find_all and count_find_new < max_find_new:
				...
				new_datas, resource_links, checked_pages = zip(*pool.map(functools.partial(globals()[f'collect_{db_name}_id_from_{resource}'], checked_id=[x[f'{resource}ID'] for x in db], checked_pages=checked_pages, timeout=start_time + timeout - time.time(), data_count=math.ceil(finding_step / process_count)), [resource_links[i::process_count] for i in range(process_count)]))
